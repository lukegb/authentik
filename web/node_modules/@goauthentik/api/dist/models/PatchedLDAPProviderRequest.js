"use strict";
/* tslint:disable */
/* eslint-disable */
/**
 * authentik
 * Making authentication simple.
 *
 * The version of the OpenAPI document: 2025.2.1
 * Contact: hello@goauthentik.io
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */
Object.defineProperty(exports, "__esModule", { value: true });
exports.instanceOfPatchedLDAPProviderRequest = instanceOfPatchedLDAPProviderRequest;
exports.PatchedLDAPProviderRequestFromJSON = PatchedLDAPProviderRequestFromJSON;
exports.PatchedLDAPProviderRequestFromJSONTyped = PatchedLDAPProviderRequestFromJSONTyped;
exports.PatchedLDAPProviderRequestToJSON = PatchedLDAPProviderRequestToJSON;
exports.PatchedLDAPProviderRequestToJSONTyped = PatchedLDAPProviderRequestToJSONTyped;
const LDAPAPIAccessMode_1 = require("./LDAPAPIAccessMode");
/**
 * Check if a given object implements the PatchedLDAPProviderRequest interface.
 */
function instanceOfPatchedLDAPProviderRequest(value) {
    return true;
}
function PatchedLDAPProviderRequestFromJSON(json) {
    return PatchedLDAPProviderRequestFromJSONTyped(json, false);
}
function PatchedLDAPProviderRequestFromJSONTyped(json, ignoreDiscriminator) {
    if (json == null) {
        return json;
    }
    return {
        'name': json['name'] == null ? undefined : json['name'],
        'authenticationFlow': json['authentication_flow'] == null ? undefined : json['authentication_flow'],
        'authorizationFlow': json['authorization_flow'] == null ? undefined : json['authorization_flow'],
        'invalidationFlow': json['invalidation_flow'] == null ? undefined : json['invalidation_flow'],
        'propertyMappings': json['property_mappings'] == null ? undefined : json['property_mappings'],
        'baseDn': json['base_dn'] == null ? undefined : json['base_dn'],
        'certificate': json['certificate'] == null ? undefined : json['certificate'],
        'tlsServerName': json['tls_server_name'] == null ? undefined : json['tls_server_name'],
        'uidStartNumber': json['uid_start_number'] == null ? undefined : json['uid_start_number'],
        'gidStartNumber': json['gid_start_number'] == null ? undefined : json['gid_start_number'],
        'searchMode': json['search_mode'] == null ? undefined : (0, LDAPAPIAccessMode_1.LDAPAPIAccessModeFromJSON)(json['search_mode']),
        'bindMode': json['bind_mode'] == null ? undefined : (0, LDAPAPIAccessMode_1.LDAPAPIAccessModeFromJSON)(json['bind_mode']),
        'mfaSupport': json['mfa_support'] == null ? undefined : json['mfa_support'],
    };
}
function PatchedLDAPProviderRequestToJSON(json) {
    return PatchedLDAPProviderRequestToJSONTyped(json, false);
}
function PatchedLDAPProviderRequestToJSONTyped(value, ignoreDiscriminator = false) {
    if (value == null) {
        return value;
    }
    return {
        'name': value['name'],
        'authentication_flow': value['authenticationFlow'],
        'authorization_flow': value['authorizationFlow'],
        'invalidation_flow': value['invalidationFlow'],
        'property_mappings': value['propertyMappings'],
        'base_dn': value['baseDn'],
        'certificate': value['certificate'],
        'tls_server_name': value['tlsServerName'],
        'uid_start_number': value['uidStartNumber'],
        'gid_start_number': value['gidStartNumber'],
        'search_mode': (0, LDAPAPIAccessMode_1.LDAPAPIAccessModeToJSON)(value['searchMode']),
        'bind_mode': (0, LDAPAPIAccessMode_1.LDAPAPIAccessModeToJSON)(value['bindMode']),
        'mfa_support': value['mfaSupport'],
    };
}
